  ...
  (function () {
    ...
    return {
      ...
      _radius: function (reach, min, max, min_r, max_r) {
        ...
        var max_sq = max * max;
        var sc =
          ((reach * 1 - min_r) / (max_r - min_r)) * (max_sq - min_sq) + min_sq;
        return Math.sqrt(sc);
      },
      ...
      _rgba: function (rgb, adj) {
        ...
        for (var i = 0; i < rgb.length; i++) {
          if (i < 3)
            col.push(Math.floor(Math.max(0, Math.min(rgb[i] + adj, 255))));
          else col.push(rgb[i]);
        }
        switch (col.length) {
          case 3:
      ...
      _random: function () {
        ...
        $.get("/labs/social/random?_cb=" + _cb)
          .done(function (data) {
          ...
          .fail(function (data) {
            $(".message")
              .addClass("error")
              .text("Error getting random ID. Please try again");
          });
      },
      _goto: function (id) {
        $("#toolbar form")
